...
- (BOOL) startRecording: (NSString *) filePath
{
    ...
    if (status) {CFRelease(fileURL); fprintf(stderr, "Could not establish new queue\n"); return NO;}
    ...
    if (status) {fprintf(stderr, "Could not create file to record audio\n"); return NO;}
...
- (CGFloat) averagePower
{
    ...
    if (status) {fprintf(stderr, "Error retrieving meter data\n"); return 0.0f;}
...
- (BOOL) resume
{
    ...
    if (status) {fprintf(stderr, "Error restarting audio queue\n"); return NO;}
    return YES;
}
